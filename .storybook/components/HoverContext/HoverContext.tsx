...
export const HoverProvider = ({
  ...
  const [, setRegisteredTargets, sharedRegisteredTargets] = useSharedState<
    Target[]
  >([])
  const [hoveredTargets, setHoveredTargets, sharedHoveredTargetIds] =
    useSharedState<Target[], string[]>([], (targets) =>
      targets.map((target) => target.id)
    )

  const updateHoveredTargets = ({
    absoluteX,
    absoluteY
    ...
    if (!areSetsEqual(prevHoveredTargetIds, newHoveredTargetIds)) {
      runOnJS(setHoveredTargets)(
        sharedRegisteredTargets.value.filter((target) =>
          newHoveredTargetIds.has(target.id)
        )
      )
    }
  }
  ...
  return (
    ...
    >
      <GestureDetector gesture={hoverGesture}>{children}</GestureDetector>
    </HoverContext.Provider>
  )
}
