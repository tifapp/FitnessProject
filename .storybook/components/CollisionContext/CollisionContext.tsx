...
import {
  doSharedMeasurementsOverlap,
  remove,
  upsert
} from "../HoverContext/utils"
...
export const CollisionProvider = ({
  ...
  const checkCollidingTargets = (
    bounds: SharedValue<Measurements>
  ): Target[] => {
    "worklet"
    const collidingTargetIds = new Set<string>()

    for (const target of sharedRegisteredTargets.value) {
      if (doSharedMeasurementsOverlap(bounds, target.measurements, hitSlop)) {
        collidingTargetIds.add(target.id)
      }
    }

    return sharedRegisteredTargets.value.filter((target) =>
      collidingTargetIds.has(target.id)
    )
  }
