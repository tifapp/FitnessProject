...
> {
	...
	send() {
		...
		Auth.forgotPassword(username.trim().toLowerCase().replace(function(c) {
	...
	submit() {
		...
		const username = this.getUsernameFromInput();
		Auth.forgotPasswordSubmit(username.trim().toLowerCase().replace(function(c) {
                return c === /[\p{L}\p{M}\p{S}\p{N}\p{P}]+/ ? c : '';
            }).replace(/\s/g,''), code, password)
			.then(data => {
				logger.debug(data);
