...
export default function ExpandingTextInputWithNameInput ({
  ...
  const isCursorInside = (position:number) => {
    ...
    const startInvisible = text.lastIndexOf("\u200a", position)
    const endInvisible = text.indexOf("\u200b", position)
    ...
    if (insideTaggedUser) {
      ...
      const distanceToStart = position - startInvisible
      const distanceToEnd = endInvisible - position

      if (distanceToEnd < distanceToStart) {
        const newPosition = endInvisible + 1
        setNewCursorPosition({ start: newPosition, end: newPosition })
        setCursorPosition([newPosition, newPosition])
      } else {
        const newPosition = startInvisible
        setNewCursorPosition({ start: newPosition, end: newPosition })
        setCursorPosition([newPosition, newPosition])
      }
    } else {
      ...
      <ExpandingTextInput
        ...
        onKeyPress={({ nativeEvent }) => {
          ...
          if (nativeEvent.key === "Backspace") {
            ...
            const startInvisible = text.lastIndexOf("\u200a", cursorPosition[0] - 1)
            const endInvisible = text.lastIndexOf("\u200b", cursorPosition[0] - 1)
            ...
            if (cursorPosition[0] === endInvisible + 1) {
              ...
              const indexTaggedUser = findTaggedUser(cursorPosition[0])
              ...
              const newTaggedUserText = text.substring(0, startInvisible) + text.substring(endInvisible + 1)
